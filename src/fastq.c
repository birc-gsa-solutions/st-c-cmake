    ...
    struct fastq_record *record)
{
    ...
    if ((iter->name->slice.len < 2) || (iter->seq->slice.len < 2))
    {
        // We didn't read anything, so abondon this and report
        // that there are no more records. A non-empty string has
        // at least 2 character, for some actual text and $.
        return false;
    }

    record->name = CSTR_SUFFIX(CSTR_SLICE_CONST_CAST(iter->name->slice), 1); // skip @
    record->seq = CSTR_PREFIX(CSTR_SLICE_CONST_CAST(iter->seq->slice), -1);  // don't include $ here
    return true;
}
